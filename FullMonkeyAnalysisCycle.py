...
def RespContDirections():
    ...
    for type_id in range(0,stimuli_number):
        ...
        psth = np.asarray(SpFun.LoadSpikeTrain(directory,data_folder, type_id))
...
def SaveDirections(directions, signal_part):
        #% save mean
        filepath = main_folder + '/out/' + data_folder + '_' + signal_part + '_direction.pickle'
...
#% save directions
# SaveDirections(control_direction, 'control')
# SaveDirections(response_direction, 'response')
SaveDirections(effect_direction, 'effect')

#%% load directions
directory = main_folder + '/out'
def LoadDirections(signal_part):
        #% save mean
        filepath = directory + '/' + data_folder + '_' + signal_part + '_direction.pickle'
        with open(filepath, 'rb') as f:
            directions = pickle.load(f)
        return directions

# control_direction = LoadDirections('control')
# response_direction = LoadDirections('response')
effect_direction = LoadDirections('effect')
#%% make graphs

def NeuroGraph(average_direction, signal_part):
    for type_id in list(range(0,stimuli_number)):
        # make direction graph
        IndegrSet, OutdegrSet, G_set = SpFun.degrSet(average_direction[type_id],True, True)
        # save graph               
        filepath = directory + '/' + data_folder + '_' + signal_part + '_' + str(type_id) +  '_graph.pickle'
        with open(filepath, 'wb') as f:
...
NeuroGraph(effect_direction, 'effect')
...
def SaveDegrees(indeg, outdeg, chosen_neuron_names):    
    filepath = directory + '/' + data_folder + '_' + signal_part +  '_degrees.pickle'
    with open(filepath, 'wb') as f:
        pickle.dump(indeg, f)
...
# #% save indeg outdeg
# SaveDegrees(indeg, outdeg, chosen_neuron_names)
...
def save_to_excel(all_outdeg, degree_dir, neuron_names):
    ...
    directory = main_folder + '/out'
    filepath = directory + '/' + data_folder + '_' + degree_dir + '_degrees.xlsx'
    to_save.to_excel(filepath)
    return to_save

saved_outdeg = save_to_excel(outdeg, 'out', chosen_neuron_names)
saved_indeg = save_to_excel(indeg, 'in', chosen_neuron_names)
...
mngr.window.setGeometry(0,30,1000,700)
